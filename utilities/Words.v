...
Lemma cast_wrd_refl : forall {X n} (w : Wrd n X), cast_wrd (eq_refl n) w = w.
...
Lemma cast_wrd_eq' : forall {X} {n1 n2} (e : n1 = n2) (x1 : Wrd n1 X) (x2 : Wrd n2 X),
  (forall k, forall (p1 : k<n1), forall (p2 : k<n2), 
    let kp1:={| val:=k; val_lt:=p1 |} in let kp2:={| val:=k; val_lt:=p2 |} in 
      x1 kp1 = x2 kp2) -> cast_wrd e x1 = x2.
Proof.
  intros X n1 n2 e x1 x2 Hkp.
  replace (x2) with (cast_wrd (eq_refl n2) x2).
  apply cast_wrd_eq.
  exact Hkp.
  exact (cast_wrd_refl x2).
Qed.
...
Lemma cast_wrd_restr_eq : forall {X} {m n} (p : m<=n) (x : Wrd n X) (e : m=n), 
...
Lemma projw_restr : forall {X} (xs : Seq X) (m n : nat) (p:m <= n),
...
Lemma restr_cat : forall {X} {n} (xw : Wrd n X) (x:X) (p:n<=S n),
...
Proof. 
  ...
  rewrite -> H. intros el. apply cast_wrd_eq'.
  intros k p1 p2 kp1 kp2. apply wrd_at_eq.
Qed.
  ...
  - intros Sln2 Sle2.
    ...
    -- simpl. 
       ...
       assert (list_to_word (proj n1 xs) = cast_wrd (eq_sym Hle1) (projw n1 xs)) as IHn1'. { 
         ...
         rewrite <- (cast_wrd_refl w1). rewrite -> cast_cast_wrd'. apply cast_wrd_eq. 
