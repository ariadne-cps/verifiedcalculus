(******************************************************************************
 *  Systems/ComposeNondeterministicBehaviours.v
 *
 ...
 *
 *  Composition of nondeterministic behaviours
 *
 ******************************************************************************)
...
Require Import InhabitedEnsembleMonad.
Require Export Causality.
Require Export BehaviourComposition.

Section NondeterministicBehaviours.


Definition NondeterministicBehaviour (U Y : Type) : Type := 
  (Seq U) -> Ensemble (Seq Y).

Definition restrict {Y : Type} (n : nat) (y : Seq Y) : Wrd n Y :=
  fun i => y i.

Definition nondeterministic_mixed_causal {UA UD Y : Type} 
  (b : @NondeterministicBehaviour (UA*UD) Y)
  : Prop :=
    forall (u u':nat->UA*UD) (n:nat),
      (forall m0:nat, m0 <= n -> fst (u m0) = fst (u' m0)) ->
      (forall m1:nat, m1 < n -> snd (u m1) = snd (u' m1)) ->
      forall w : Wrd n Y, 
        exists y : Seq Y, restrict n y = w /\ b u y <->
        exists y' : Seq Y, restrict n y' = w /\ b u' y'.
...
Definition is_composed_nondeterministic_behaviour {UA UD Y1 Y2 : Type}
  (b1 : @NondeterministicBehaviour (UA*(UD*Y2)) Y1)
  (b2 : @NondeterministicBehaviour ((UA*Y1)*UD) Y2)
  (b12 : @NondeterministicBehaviour (UA*UD) (Y1*Y2))
  : Prop :=
  forall (u : Seq (UA*UD)),

     (* Separate inputs *)
     let ua : (Seq UA) := (fun k => fst (u k)) in
     let ud : (Seq UD) := (fun k => snd (u k)) in

     (* Candidate output *)
     forall y12 : Seq (Y1 * Y2),
       (* Separate outputs *)
       let y1 : (Seq Y1) := (fun k => fst (y12 k)) in
       let y2 : (Seq Y2) := (fun k => snd (y12 k)) in
         (* Generate inputs *)       
         let u1 := fun k => (ua k, (ud k, y2 k)) in
         let u2 := fun k => ((ua k, y1 k), ud k) in
           (* Composition condition *)
           b12 u y12 <-> b1 u1 y1 /\ b2 u2 y2
.

Definition compose_nondeterministic_behaviours_noinputs
  {Y1 Y2 : Type}
  (b1 : @NondeterministicBehaviour Y2 Y1)
  (b2 : @NondeterministicBehaviour Y1 Y2)
  (y_default : Y1)
  : Ensemble (Seq (Y1*Y2)) :=
    fun y12 : Seq (Y1*Y2) => 
      let y1 : Seq Y1 := (fun k => fst (y12 k)) in
...
Definition compose_nondeterministic_behaviours {UA UD Y1 Y2 : Type}
  (b1 : @NondeterministicBehaviour (UA*(UD*Y2)) Y1)
  (b2 : @NondeterministicBehaviour ((UA*Y1)*UD) Y2)
  : @NondeterministicBehaviour (UA*UD) (Y1*Y2) :=
  fun (u : Seq (UA*UD)) => 
     (* Separate inputs *)
     let ua : (Seq UA) := (fun k => fst (u k)) in
     let ud : (Seq UD) := (fun k => snd (u k)) in
...
End NondeterministicBehaviours.



Section CopyBehaviours.
