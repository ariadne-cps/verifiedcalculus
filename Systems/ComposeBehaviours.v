...
Notation is_composed_behaviour := BehaviourComposition.is_composed_behaviour.
...
Lemma zip_unzip3_l {Y1 Y2 Y3} :
  forall (y123 : Tr ((Y1*Y2)*Y3)),
    y123 ≡ ((fst (unzip (fst (unzip y123))); 
             snd (unzip (fst (unzip y123)))); 
            snd (unzip y123)).
Proof.
  intros y123. 
  transitivity (fst (unzip y123); snd (unzip y123)).
  symmetry; now apply zip_unzip.
  apply zip_prod_r. symmetry; now apply zip_unzip.
Qed.
Lemma zip_unzip3_r {Y1 Y2 Y3} :
  forall (y123 : Tr (Y1*(Y2*Y3))),
    y123 ≡ (fst (unzip y123);
            (fst (unzip (snd (unzip y123))); 
             snd (unzip (snd (unzip y123))))).
Proof.
  intros y123. 
  transitivity (fst (unzip y123); snd (unzip y123)).
  symmetry; now apply zip_unzip.
  apply zip_prod_l. symmetry; now apply zip_unzip.
Qed.
