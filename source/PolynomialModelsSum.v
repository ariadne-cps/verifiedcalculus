...
Definition error_add t1 t2 : F := Fadd_up t1.(error) (Fadd_up t2.(error) (Fsum_snd_add up (pre_merge_add_error t1.(polynom) t2.(polynom)))).

Definition pre_merge_add_polynom p1 p2 : list (nat*F) := pre_merge (p1,p2).
...
Definition PMadd (t1 t2 : PolynomialModel) : PolynomialModel :=
 {| polynom:= pre_merge_add_polynom t1.(polynom) t2.(polynom); 
    polynom_sorted:= @pre_merge_sorted t1.(polynom) t2.(polynom) t1.(polynom_sorted) t2.(polynom_sorted);
    error:=error_add t1 t2 |}.

Lemma sum_add_up_nonneg:forall (t1 t2: PolynomialModel), 0<= FinjR ( Fsum_snd_add up (pre_merge_add_error (polynom t1) (polynom t2))).
Proof.
 intros [p1 H1 e1].
...
Lemma sum_add_up_property : forall (p1 p2:Polynomial) x,  -1 <= x <= 1 ->
   Rabs (Pax_eval p1 x + Pax_eval p2 x - Pax_eval (pre_merge_add_polynom p1 p2) x) <=
...
*)
  induction p1; intros p2.
    intros  x _; simpl; unfold pre_merge_add_polynom, pre_merge_add_error in *.
    ...
    induction p2.
      ...
      intros x Hx.
      unfold pre_merge_add_polynom, pre_merge_add_error.
      ...
      destruct (lt_eq_lt_dec (fst a) (fst a0)) as [[Hlt | Hlt] | Hlt].
        ...
        replace (Pax_eval ((fst a, snd a) :: pre_merge (p1, a0 :: p2)) x) with
                (FinjR (snd a) *(pow x (fst a)) + (Pax_eval (pre_merge_add_polynom p1 (a0::p2)) x)); [|trivial].
        replace (pre_merge_error (p1, a0 :: p2)) with
                (pre_merge_add_error p1 (a0::p2)); [|trivial].
        stepl (Rabs (Pax_eval p1 x +
                     Pax_eval (a0::p2) x -
                     Pax_eval (pre_merge_add_polynom p1 (a0::p2)) x)).
          apply (IHp1 (a0::p2)). exact Hx.
          remember (pre_merge_add_polynom p1 (a0::p2)) as p12.
        ...
        stepl (Rabs ( ( (FinjR (snd a) + FinjR (snd a0)) * (pow x (fst a)) -
                      ...
                      (Pax_eval p1 x + Pax_eval p2 x - Pax_eval (pre_merge_add_polynom p1 p2) x)
                    ) ).
        2:f_equal; simpl; unfold pre_merge_add_polynom; simpl; ring.
        ...
        replace (Pax_eval ((fst a0, snd a0) :: pre_merge (a :: p1, p2)) x) with
                (FinjR (snd a0) *(pow x (fst a0)) + (Pax_eval (pre_merge_add_polynom sp1 sp2) x)); [|trivial].
        replace (pre_merge_error (a :: p1, p2)) with
                (pre_merge_add_error sp1 sp2); [|trivial].
        stepl (Rabs (Pax_eval sp1 x +
                     Pax_eval sp2 x -
                     Pax_eval (pre_merge_add_polynom sp1 sp2) x)).
          apply IHp2. exact Hx.
          remember (pre_merge_add_polynom sp1 sp2) as sp12.
          f_equal. simpl. ring.
Qed.
...
Proof.
 ...
 apply Rle_trans with (FinjR (error t1)+ FinjR (error t2) + FinjR (Fsum_snd_add up (pre_merge_add_error (polynom t1) (polynom t2)))).

  2:apply Rle_trans with (FinjR (error t1)+ FinjR (error t2) + FinjR (Fsum_snd_add up (pre_merge_add_error (polynom t1) (polynom t2))));
     [ apply Rplus_le_compat_l;
       generalize (FinjR (Fsum_snd_add up (pre_merge_add_error (polynom t1) (polynom t2)))) H_sum_err_nonneg; intros r H_r; lra
     | apply Rle_trans with (FinjR (error t1)+ FinjR (Fadd_up (error t2) (Fsum_snd_add up (pre_merge_add_error (polynom t1) (polynom t2)))));
       [(stepl (FinjR (error t1)+ (FinjR (error t2) + FinjR (Fsum_snd_add up (pre_merge_add_error (polynom t1) (polynom t2))))) by ring) ;
 ...
 set (pp_x:= Pax_eval (pre_merge_add_polynom p1 p2) x).
