(******************************************************************************
 *  Utilities/SetMonads.v
 *
 *  Copyright 2023-6 Pieter Collins
 *
 ******************************************************************************)
...
Class Monad (M : Set -> Set) :=
{
    ...
    Mpure : forall {A : Set}, A -> M A;
    Mbind : forall {A B : Set}, (A -> M B) -> M A -> M B;

    (* coherence conditions *)
    Mleft_identity : forall {A B : Set} (f:A->M B) (a:A), Mbind f (Mpure a) = f a;
    Mright_identity : forall {A : Set} (x : M A), Mbind (@Mpure A) x = x;
    Massociativity : forall {A B C : Set} (x : M A) (f : A -> M B) (g : B -> M C),
        Mbind g (Mbind f x) = Mbind (fun a => Mbind g (f a)) x;

    (* Mfunctor_map : forall {A B : Set}, (A -> B) -> M A -> M B; *)
    Mfunctor_map {A B : Set} : (A -> B) -> M A -> M B
      := fun (f : A -> B) (x : M A) => Mbind (fun x' => Mpure (f x')) x;
    Mlift {A B : Set} (f : A -> B) (a : M A) : M B
      := Mbind (fun a' => Mpure (f a')) a;

   Mleft_product {X Y : Set} : M X -> M Y -> (M (prod X Y)) :=
      fun (mu : M X) (nu : M Y) => Mbind ( fun y => ( Mbind (fun x => Mpure (pair x y)) mu ) ) nu;
   Mright_product {A B : Set} : M A -> M B -> M (prod A B)
...
Theorem Mlift_associative : forall {M} {MonadM : Monad M} {A:Set} {B:Set} {C:Set} (a : M A) (f:A->B) (g:B->C),
...
Lemma Mlift_identity {M} {_ : Monad M}: forall {A : Set},
...
Lemma Mfunctorial_compose {M} {_ : Monad M}  : forall {A B C : Set} (f : A -> B) (g : B -> C),
...
Lemma Mfunctorial_identity {M} {_ : Monad M}: forall {A : Set},
...
Definition Mjoin {M} {_ : Monad M} {A : Set}
  := fun (mma : M (M A)) => Mbind (fun (ma : M A) => ma) mma.

(* Notation "unit" := pure; *)
Definition Munit {M} {MonadM} {A : Set} := @Mpure M MonadM A.
(* Notation "join" := mult; *)
Definition Mmult {M} {MonadM} {A : Set} := @Mjoin M MonadM A.

Definition Mcompose {M} {_ : Monad M} {A B C : Set} (g : B -> M C) (f : A -> M B) : A -> M C
  := fun (a : A) => Mbind g (f a).
...
Lemma Mjoin_natural_transormation {M} {_ : Monad M} : forall {A B : Set} (f : A -> B) (al : M A),
...
Theorem Mproduct_pure_pure {M} {_ : Monad M} : forall {A:Set} {B:Set} (x : A) (y : B),
...
Theorem Mproduct_pure_monad {M} {MonadM  : Monad M} : forall {A:Set} {B:Set} (x : A) (b : M B),
...
(* Requires functional_extensionality *)
Theorem Mproduct_monad_pure {M} {MonadM  : Monad M} : forall {A:Set} {B:Set} (a : M A) (y : B),
...
Definition Mright_skew {M} {MonadM : Monad M} {A B : Set} (mu : M A) (nu : A -> M B) : M (prod A B) :=
...
Definition Mbind_const (M : Set -> Set) (MonadM : Monad M) : Prop
  := forall (A B : Set) (b: M B) (x : M A),
       Mbind (fun (a:A) => b) x = b.

Lemma Mright_skew_first {M} {MonadM : Monad M} :
  Mbind_const M MonadM -> forall {A B : Set} mu nu,
...
Lemma Mlift_compose {M} {MonadM : Monad M} :
  forall {A B C : Set} (f:A->B) (G:B->M C) (x:M A),
...
Definition tuple_associate {A B C : Set} (a_bc : prod A (prod B C)) : prod (prod A B) C
  := ((fst a_bc, fst (snd a_bc)), snd (snd a_bc)).

Definition tuple_unassociate {A B C : Set} (ab_c : prod (prod A B) C) : prod A (prod B C)
  := (fst (fst ab_c), ( snd (fst ab_c), snd ab_c) ).

Proposition Mright_skew_associative {M} {MonadM : Monad M} :
  forall {A B C : Set} (x : M A) (f : A -> M B) (g : (prod A B) -> M C),
