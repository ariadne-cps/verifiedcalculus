...
Check @Kleeneans.indeterminate nat Topologic.nat_Lattice.

Definition PKindeterminate := @Kleeneans.indeterminate nat Topologic.nat_Lattice. 
Definition Kindeterminate : Kleeneans.Kleenean := PKindeterminate.
...
Notation Tribool := Kleeneans.Tribools.Tribool.
Notation Kleenean := Kleeneans.Kleenean.

Notation indt := Kleeneans.Tribools.indt.
...
Lemma make_monotone_correct : forall s : nat -> Tribool,
  @Kleeneans.is_monotone nat (Topologic.nat_Lattice) (make_monotone s).
Proof.
  intros s. 
  unfold Kleeneans.is_monotone.
  unfold Topologic.BasicTopologic.is_monotone.
  unfold Topologic.BasicTopologic.refines.
  simpl.
  assert (forall m k, Kleeneans.Tribools.refines (make_monotone s (m+k)) (make_monotone s m)) as Hplus. {
    intros m. induction k.
    - replace (m+0)%nat with m.
      apply Kleeneans.Tribools.refines_refl.
      now rewrite -> Nat.add_0_r. 
    - apply (Kleeneans.Tribools.refines_trans _ (make_monotone s (m+k)) _).
      2: exact IHk.
      replace (m+S k)%nat with (S (m+k))%nat.
      2: now rewrite <- Nat.add_succ_r.
      
      remember (make_monotone s (m+k)) as msn.
      destruct msn.
      -- unfold Kleeneans.Tribools.refines.
         destruct (make_monotone s (Kleeneans.succ (m + k))). tauto. destruct b; tauto.
  ...
  pose proof (Nat.sub_add m n Hmn).
  unfold k.
...
Definition make_kleenean (s : nat -> Tribool) : Kleenean :=
  Kleeneans.mkPreKleenean (make_monotone s) (make_monotone_correct s).

Definition CRls (r1 r2 : CReal) : Kleeneans.Kleenean :=
  let tbs :=
    fun n : nat =>
      ...
      let Hgtle := Qlt_le_dec (seq r2 z + 2 ^ z) (seq r1 z - 2 ^ z) in
        match Hltge with
        | left _ => Kleeneans.Tribools.tru
        | right _ =>
              match Hgtle with 
              | left _ => Kleeneans.Tribools.fls
              | right _ => Kleeneans.Tribools.indt
              end
        end in
          make_kleenean tbs  
.
